...
void money() 
{
	INT_PTR PlayerCompPtr = Read<UINT_PTR>(base_address + 0x10A9FBC8);
	if (GetAsyncKeyState(VK_F1))
	{
		printf("money set\n");
		write<ULONG>(PlayerCompPtr + 0x5CE4 + 0x20, 100000);
		write<ULONG>(PlayerCompPtr + 0x5CE4 + 0xB830 + 0x20, 100000);
		write<ULONG>(PlayerCompPtr + 0x5CE4 + (2 * 0xB830) + 0x20, 100000);
		write<ULONG>(PlayerCompPtr + 0x5CE4 + (3 * 0xB830) + 0x20, 100000);
...
void xpgun()
{
	if (GetAsyncKeyState(VK_F4))
	{
		...
		if (xp == true) {
			...
			write<FLOAT>(base_address + 0x10AC7BE0 + 0x08, 2000);

			write<FLOAT>(base_address + 0x10AC7BE0 + 0x10, 500);
		...
		{
			printf("xp off\n");
			write<FLOAT>(base_address + 0x10AC7BE0 + 0x08, 1);
			write<FLOAT>(base_address + 0x10AC7BE0 + 0x10, 1);
